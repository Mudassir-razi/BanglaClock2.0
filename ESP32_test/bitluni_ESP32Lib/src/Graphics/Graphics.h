...
class Graphics: public ImageDrawer
{
  public:
	...
	void setCustomFont(FontVar &font)
	{
	...
	void printCustom(int ch)
	{
		int char_width, char_height, cursorAdvance, bitmapOffset;
		int x_offset, y_offset;
	
		if(!fontCustom)return;
		else if(!fontCustom->data) return;
		else
		{
			int off = fontCustom->firstChar;
			int dataIndex = (ch - off) * 4;
			bitmapOffset = fontCustom->data[dataIndex + 0];
			char_width = fontCustom->data[dataIndex + 1];
			char_height = fontCustom->data[dataIndex + 2];
			cursorAdvance = fontCustom->data[dataIndex + 3];
		}
		
		//sprintf(log, "%d %d %d", bitmapOffset, char_width, char_height);
		
		if (fontCustom->valid(ch))
		{
			const unsigned char *pix = &fontCustom->pixels[bitmapOffset];
			for (int py = 0; py < char_height * textScale; py += textScale){
				for (int px = 0; px < char_width * textScale; px += textScale){
					if (*(pix++)) 
					{
						for(int i = 0;i < textScale;i++)
						{
							for(int j = 0;j < textScale;j++)
							{
								dotMix(px + cursorX + i, py + cursorY + j, frontColor);
							}
						}
					}
					//else dotMix(px + cursorX, py + cursorY, backColor);
				}
			}
			cursorX += cursorAdvance * textScale;
			if(cursorX > xres)
			{
				cursorX = cursorBaseX;
				cursorY += char_height * textScale;
			}
		}
		else
		{
	...
	void printlnCustom(const char *ch)
	{
		while(*ch)
		{
			printCustom((int)(*ch));
